...
def calculate_ssim(imgA, imgB, gray_scale=False):
    ...
    # return score
    min_dim = min(imgA.shape[:2])

    # Ensure win_size is an odd value and does not exceed the smallest dimension
    win_size = min(7, min_dim if min_dim % 2 != 0 else min_dim - 1)

    # Compute SSIM
    score, diff = SSIM(imgA, imgB, full=True, win_size=win_size, channel_axis=-1)
    return score
