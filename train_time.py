...
def main():
    ...
    while current_step < n_iter:
        ...
        for _, train_data in enumerate(train_loader):
            ...
            # the below code change Zhan
            # log

            iteration_time = time.time() - start_time
            logs = diffusion.get_current_log()
            logger.info(f'{current_step} took {iteration_time:.6f} seconds')
            # if current_step % 1 == 0 and rank <= 0:
            #     logs = diffusion.get_current_log()
            ...
            # if current_epoch % 10 == 0 and rank <= 0:
            if current_step % 10000 == 0 and rank <= 0:
                logger.info(f'Saving models at epoch {current_epoch}')
            ...
            if current_step % opt['train']['val_freq'] == 0 and rank <= 0:
                ...
                for val_data in val_loader:
                    ...
                    if img_mode == 'single':
                        ...
                        util.save_img(
                            Metrics.tensor2img(visuals['HQ'][-1]),
                            '{}/{}_{}_normal.png'.format(result_path, current_step, idx))
